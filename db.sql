...
CREATE TABLE
    pesan (
        ...
        id_company INT,
        FOREIGN KEY (id_company) REFERENCES company_authprofil(id),
        id_worker INT,
        FOREIGN KEY (id_worker) REFERENCES workers_authprofile(id),
        company_message VARCHAR,
        workers_message VARCHAR,
...
(
    SELECT
        jsonb_agg(
            jsonb_build_object(
                'id',
                workers_experience.id,
                'id_workers',
                workers_experience.id_workers,
                'posisi',
                workers_experience.posisi,
                'nama_perusahaan',
                workers_experience.nama_perusahaan,
    ...
    FROM workers_experience
    WHERE
        workers_experience.id_workers = workers_authprofile.id
) AS experience,
...
(
    SELECT
        jsonb_agg(
            jsonb_build_object(
                'id',
                workers_portofolio.id,
                'id_worker',
                workers_portofolio.id_worker,
                'porto_name',
                workers_portofolio.porto_name,
                'porto_link',
                workers_portofolio.porto_link,
    ...
    FROM workers_portofolio
    WHERE
        workers_portofolio.id_worker = workers_authprofile.id
) AS portofolio

FROM workers_authprofile
    LEFT JOIN workers_skills ON workers_authprofile.id = workers_skills.id_worker
WHERE
    workers_authprofile.id = ${id}
GROUP BY
    workers_authprofile.id,
    workers_authprofile.username,
    workers_authprofile.email,
